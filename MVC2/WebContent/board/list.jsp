	...
	<center>
				...
				<c:forEach var="article" items="${articleList }">
					<tr height="30">
						...
						<td width="250"><c:if test="${article.depth> 0 }">
							...
							href="/MVC2/board/content.do?num=${article.num }&pageNum=${currentPage }">
		...
		<c:if test="${count > 0 }">
			...
			<c:if test="${startPage > pageBlock }">
				<a href="/MVC2/board/list.do?pageNum=${startPage - pageBlock }">[이전]</a>
			</c:if>
			<c:forEach var="i" begin="${startPage }" end="${endPage}">
				<a href="/MVC2/board/list.do?pageNum=${i}">[${i}]</a>
			</c:forEach>
			<c:if test="${endPage < pageCount }">
				<a href="/MVC2/board/list.do?pageNum=${startPage + pageBlock }">[다음]</a>
			</c:if>
		</c:if>
	</center>
</body>
